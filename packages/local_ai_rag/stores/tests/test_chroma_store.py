...
class TestAddAndSearch:
    ...
    async def test_identical_vector_scores_close_to_one(self):
        store = make_store("test-identical-score")
    ...
    async def test_search_respects_k(self):
        store = make_store("test-respects-k")
        for i in range(5):
            await store.add(f"d{i}", f"text {i}", np.array([1.0, float(i)]))
        results = await store.search(np.array([1.0, 0.0]), k=3)
        assert len(results) == 3

    async def test_search_result_carries_text_and_metadata(self):
        store = make_store("test-text-metadata")
...
class TestDelete:
    async def test_removes_the_document(self):
        store = make_store("test-delete")
        await store.add("d1", "text", np.array([1.0, 0.0]))
        await store.delete("d1")
        assert await store.count() == 0

    async def test_deleted_document_is_absent_from_search_results(self):
        store = make_store("test-delete-search")
...
class TestMetadataFilter:
    async def test_excludes_nonmatching_documents(self):
        store = make_store("test-filter-single")
    ...
    async def test_requires_all_keys_to_match(self):
        store = make_store("test-filter-multi")
...
class TestPersistence:
    async def test_persists_across_a_new_client_against_the_same_path(self, tmp_path):
        path = str(tmp_path / "chroma-db")
        store1 = ChromaVectorStore("persisted", path=path)
        await store1.add("d1", "text", np.array([1.0, 0.0]))

        store2 = ChromaVectorStore("persisted", path=path)
