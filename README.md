...
| ✅ 3. Local engineering assistant | Repo-aware coding CLI (explain, search, generate tests, suggest refactors, propose/apply patches with approval, run tests) — reuses Module 17's real AST search/patch/test tools; all 6 curriculum failure cases (invented file path, unrelated file change, unsafe command, invalid patch, missing import, unrunnable test) proven caught for real against a richer demo repo with a genuine bug. See [projects/03_engineering_assistant/](projects/03_engineering_assistant/). | §36 |
